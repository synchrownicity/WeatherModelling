...
# 12_0_model_weights
#regex
def lstm_model(N, delays, base_lr=[0.0005], train_val_X=train_val_X, train_val_y=train_val_y, test_X=test_X, test_y=test_y):
    for delay in delays:
        # In each delay
        for n in N:
            ...
            for idx, (train_index, val_index) in enumerate(tss.split(train_val_X)):
                ...
                for lr in base_lr:
                    ...
                    model.save(f"LSTM model_N{n}_delay{delay}_{idx}th fold_base lr{lr}.h5")  # Saves the whole model including architecture and weights

                    # 3. Saving the training, validation, test history for each fold (RMSE, MAE, etc.)
                    with open(f"LSTM history_N{n}_delay{delay}_{idx}th fold_base lr{lr}.json", "w") as f:
                        json.dump(history.history, f)


lstm_model(N=N, delays=delays)
